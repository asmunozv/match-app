...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 $st=$db->prepare("SELECT id,password_hash FROM users WHERE email=?"); $st->bind_param("s",$email); $st->execute(); $u=$st->get_result()->fetch_assoc();
 if($u && password_verify($pass,$u['password_hash'])){session_regenerate_id(true);$_SESSION['user_id']=$u['id'];header('Location: discover.php');exit;}
